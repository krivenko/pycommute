 ...
 *
 * Copyright (C) 2020-2026 Igor Krivenko
 *
...
void register_elementary_space(py::module_ & m) {
  ...
  .def("__eq__",
       ...
       "Equality of elementary spaces "
       "(the spaces correspond to the same quantum degree of freedom).",
       py::is_operator(),
    py::arg("es2")
  )
  .def("__ne__",
       [](es_type const& es1, es_type const& es2){ return es1 != es2; },
       "Inequality of elementary spaces "
       "(the spaces correspond to different quantum degrees of freedom).",
       py::is_operator(),
       py::arg("es2")
  )
  .def("__lt__",
       [](es_type const& es1, es_type const& es2){ return es1 < es2; },
       "Less-comparison of elementary spaces "
       "(comparison of algebra IDs followed by the algebra-specific "
       "comparison rule if the IDs coincide).",
       py::is_operator(),
       py::arg("es2")
  )
  .def("__gt__",
       [](es_type const& es1, es_type const& es2){ return es1 > es2; },
       "Greater-comparison of elementary spaces "
       "(comparison of algebra IDs followed by the algebra-specific "
       "comparison rule if the IDs coincide).",
       py::is_operator(),
       py::arg("es2")
...
:param indices: Index sequence of the corresponding creation/annihilation
                operator.
)=",
    py::arg("indices")
  )
...
:param dim_boson: Dimension of every bosonic elementary space to be constructed.
  ...
  .def(py::self == py::self, "Equivalence of Hilbert spaces.", py::arg("hs"))
  .def(py::self != py::self, "Inequivalence of Hilbert spaces.", py::arg("hs"))
  .def("add",
    &hs_type::add,
R"=(
...
:param es: Elementary space to add.
  ...
  .def("bit_range",
    &hs_type::bit_range,
R"=(
Bit range (lowest bit, highest bit) spanned by a given elementary space in
the direct product.
...
:param es: Elementary space corresponding to the selected index in
           the decomposition.
...
expression in a Hilbert space.

:param expr: Source polynomial expression.
:param hs: Hilbert space the linear operator acts in.
)=",
...
This function can optionally collect non-vanishing matrix elements
of :math:`\hat O` and :math:`\hat O^\dagger` and return them in the form of
...
void register_loperator_call_csv(py::class_<LOpType> & lop) {
  ...
  auto docstring = "\nAct on a view of a " + src_vector_text +
    " compressed state vector and write the result through a view of another " +
    dst_vector_text +
    " state vector.\n" +
    R"=(
:param src: View of the source state vector.
